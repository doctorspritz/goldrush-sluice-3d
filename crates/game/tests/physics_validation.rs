...
use game::editor::{EditorLayout, GutterPiece, Rotation, ScenarioConfig, ScenarioSpawnConfig};
...
impl TestHarness {
    ...
    fn spawn_dem_particles_random(
        ...
        for _ in 0..count {
            ...
            self.dem_sim
                .spawn(template_idx, Vec3::new(x, y, z), Vec3::ZERO);
        }
    }
        ...
        for frame in 0..num_frames {
            {
                ...
                if frame == 0 {
                    ...
                    println!(
                        "DEM sim bounds: {:?} to {:?}",
                        self.dem_sim.bounds_min, self.dem_sim.bounds_max
                    );
                    println!(
                        "DEM sim use_dem: {}, gravity: {:?}",
                        self.dem_sim.use_dem, self.dem_sim.gravity
                    );
                    println!(
                        "DEM sim stiffness: normal={}, tangential={}",
                ...
                if frame == 0 && !self.dem_sim.clumps.is_empty() {
                    println!(
                        "After step 0: particle[0] pos={:?}, vel={:?}",
    ...
    fn avg_particle_velocity(&self) -> f32 {
        ...
        self.dem_sim
            ...
            .sum::<f32>()
            / self.dem_sim.clumps.len() as f32
    }
    ...
    fn finalize(&mut self, passed: bool) {
        self.report_lines
            .push(format!("Result: {}", if passed { "PASS" } else { "FAIL" }));
        self.report_lines
            .push(format!("Frames: {}", self.frame_count));
        self.write_report();
    }
}
...
// ============================================================================
// SCENARIO HELPERS (Washplant Editor parity)
// ============================================================================
...
fn build_multigrid_headless(layout: &EditorLayout) -> MultiGridSim {
    let mut sim = MultiGridSim::new();

    for (idx, gutter) in layout.gutters.iter().enumerate() {
        sim.add_gutter_headless(gutter, idx);
    }
    for (idx, sluice) in layout.sluices.iter().enumerate() {
        sim.add_sluice_headless(sluice, idx);
    }
    for (idx, deck) in layout.shaker_decks.iter().enumerate() {
        sim.add_shaker_deck_headless(deck, idx);
    }

    sim
}

fn spawn_dem_from_config(sim: &mut MultiGridSim, spawn: &ScenarioSpawnConfig) {
    if spawn.count == 0 {
        return;
    }

    let mut rng = SimpleRng::new(spawn.seed);
    let template_idx = if spawn.use_gold {
        sim.gold_template_idx
    } else {
        sim.sand_template_idx
    };

    for _ in 0..spawn.count {
        let x = spawn.min.x + rng.next_float() * (spawn.max.x - spawn.min.x);
        let y = spawn.min.y + rng.next_float() * (spawn.max.y - spawn.min.y);
        let z = spawn.min.z + rng.next_float() * (spawn.max.z - spawn.min.z);
        sim.dem_sim
            .spawn(template_idx, Vec3::new(x, y, z), spawn.velocity);
    }
}

fn average_dem_axis_velocity(sim: &MultiGridSim, axis: char) -> f32 {
    if sim.dem_sim.clumps.is_empty() {
        return 0.0;
    }
    ...
    sum / sim.dem_sim.clumps.len() as f32
}
...
fn test_dem_floor_collision() {
    ...
    println!(
        "Gutter width: {} (inlet) to {} (outlet)",
        gutter.width, gutter.end_width
    );
    ...
    harness
        .report_lines
        .push(format!("Max penetration: {:.6}m", max_penetration));
    harness
        .report_lines
        .push(format!("Avg velocity: {:.6} m/s", final_avg_vel));
    ...
    println!(
        "Actual penetration below expected rest: {:.4}m",
        actual_penetration
    );
    ...
    assert!(
        actual_penetration < 0.005,
        "Floor penetration {:.4}m exceeds 5mm limit",
        actual_penetration
    );
    assert!(settled, "Particles not settled");
    assert!(
        final_avg_vel < 0.05,
        "Avg velocity {:.6} too high (limit 0.05 m/s)",
        final_avg_vel
    );
...
fn test_dem_wall_collision() {
    ...
    println!(
        "Gutter center: {:?}, width: {}, half_width: {}",
        gutter_pos, gutter_width, half_width
    );
    ...
    harness
        .dem_sim
        .spawn(harness.gold_template_idx, pos1, Vec3::new(0.0, 0.0, 0.5));
    ...
    harness
        .dem_sim
        .spawn(harness.gold_template_idx, pos2, Vec3::new(0.0, 0.0, -0.5));
    ...
    let max_escape = if max_z_reached > half_width {
        max_z_reached - half_width
    } else {
        0.0
    };

    println!(
        "Max Z distance from center: {:.4}m, Half-width: {:.4}m",
        max_z_reached, half_width
    );
    ...
    harness
        .report_lines
        .push(format!("Max Z reached: {:.6}m", max_z_reached));
    harness
        .report_lines
        .push(format!("Max wall escape: {:.6}m", max_escape));
    ...
    assert!(
        max_escape < 0.02,
        "Particles escaped through wall: {:.4}m",
        max_escape
    );
...
fn test_dem_density_separation() {
    ...
    harness
        .report_lines
        .push(format!("Avg gold Y: {:.4}m", avg_gold_y));
    harness
        .report_lines
        .push(format!("Avg sand Y: {:.4}m", avg_sand_y));

    let passed = avg_gold_y < avg_sand_y;
    harness.finalize(passed);

    assert!(
        avg_gold_y < avg_sand_y,
        "Gold ({:.4}) should settle below sand ({:.4})",
        avg_gold_y,
        avg_sand_y
    );
...
fn test_dem_settling_time() {
    ...
    harness.spawn_dem_particles(1, spawn_pos, Vec3::ZERO, true);
    println!(
        "Spawned single particle at Y={:.3} (50cm above floor at Y={:.3})",
        spawn_pos.y, floor_y
    );
    ...
    harness.report_lines.push(format!(
        "Settled at 5s: {} (vel={:.4})",
        settled_at_5s, avg_vel_at_5s
    ));
    harness
        .report_lines
        .push(format!("Max vel after 8s: {:.6} m/s", max_vel_after_8s));
    ...
    assert!(
        settled_at_5s,
        "Not settled within 5 seconds (avg_vel={:.4})",
        avg_vel_at_5s
    );
    assert!(
        max_vel_after_8s < 0.2,
        "Particles still moving after 8s: {:.4} m/s",
        max_vel_after_8s
    );
...
fn test_fluid_flow_direction() {
    let scenario = load_scenario("test_scenarios/05_flow_direction.json");
    let test = scenario
        .test
        .as_ref()
    ...
    let checks = test
        ...
        .flow_direction
        .as_ref()
        .expect("Scenario missing flow_direction check");

    let mut sim = build_multigrid_headless(&scenario.layout);
    spawn_dem_from_config(&mut sim, spawn);

    let duration = test.duration_s.unwrap_or(5.0);
    let steps = (duration / DT) as usize;
    for _ in 0..steps {
        sim.step_headless(DT);
    }
    ...
    let axis = checks
        ...
        .unwrap_or('x');
    let avg_axis = average_dem_axis_velocity(&sim, axis);
    let min_avg = checks.min_avg_vel.unwrap_or(0.01);

    let gutter = scenario
        .layout
        .gutters
        .first()
        .expect("Scenario missing gutter");
    let base_dir = match gutter.rotation {
        Rotation::R0 => Vec3::X,
        Rotation::R90 => Vec3::Z,
        Rotation::R180 => -Vec3::X,
        Rotation::R270 => -Vec3::Z,
    };
    let slope_sign = gutter.angle_deg.signum();
    let flow_dir = base_dir * slope_sign;
    let expected_sign = match axis {
        'x' | 'X' => flow_dir.x.signum(),
        'y' | 'Y' => flow_dir.y.signum(),
        'z' | 'Z' => flow_dir.z.signum(),
        _ => flow_dir.x.signum(),
    };

    let passed = if expected_sign == 0.0 {
        avg_axis.abs() > min_avg
    } else {
        expected_sign == avg_axis.signum() || avg_axis.abs() > min_avg
    };
...
fn test_fluid_pool_equilibrium() {
    let scenario = load_scenario("test_scenarios/06_pool_equilibrium.json");
    let test = scenario
        .test
        .as_ref()
        .expect("Scenario missing test config");
    ...
    let checks = test
        ...
        .pool_equilibrium
        .as_ref()
        .expect("Scenario missing pool_equilibrium check");

    let mut sim = build_multigrid_headless(&scenario.layout);
    spawn_dem_from_config(&mut sim, spawn);

    let duration = test.duration_s.unwrap_or(8.0);
    let steps = (duration / DT) as usize;
    for _ in 0..steps {
        sim.step_headless(DT);
    }
    ...
    let sum_sq = sim
        ...
        .sum::<f32>();
    let rms_vel = (sum_sq / sim.dem_sim.clumps.len().max(1) as f32).sqrt();
    let max_rms = checks.max_rms_vel.unwrap_or(0.3);

    assert!(
        rms_vel < max_rms,
        "Pool not at equilibrium: RMS vel {:.6} (max {:.3})",
        rms_vel,
        max_rms
    );

    println!("✓ TEST 6: Fluid Pool Equilibrium - PASSED (scenario)");
}

#[test]
fn test_fluid_wall_containment() {
    let scenario = load_scenario("test_scenarios/07_wall_containment.json");
    let test = scenario
        .test
        .as_ref()
        .expect("Scenario missing test config");
    ...
    let checks = test
        ...
        .wall_containment
        .as_ref()
        .expect("Scenario missing wall_containment check");
    ...
    let gutter = scenario
        ...
        .first()
        .expect("Scenario missing gutter");
    let half_width = gutter.width / 2.0;
    let max_escape = checks.max_escape.unwrap_or(0.02);
    ...
    for _ in 0..steps {
        ...
        for clump in &sim.dem_sim.clumps {
            let deviation = match gutter.rotation {
                Rotation::R0 | Rotation::R180 => (clump.position.z - gutter.position.z).abs(),
                Rotation::R90 | Rotation::R270 => (clump.position.x - gutter.position.x).abs(),
            };
            max_lateral_deviation = max_lateral_deviation.max(deviation);
        }
    }

    let escape_amount = (max_lateral_deviation - half_width).max(0.0);

    assert!(
        escape_amount < max_escape,
        "Particles escaped walls by {:.4}m (max {:.4}m)",
        escape_amount,
        max_escape
    );

    println!("✓ TEST 7: Fluid Wall Containment - PASSED (scenario)");
}
...
fn test_sediment_settling() {
    ...
    harness.run_for_seconds(10.0, |h, _| {
        final_y = h.dem_sim.clumps.iter().map(|c| c.position.y).sum::<f32>()
            / h.dem_sim.clumps.len().max(1) as f32;
    ...
    harness
        .report_lines
        .push(format!("Start Y: {:.4}m", start_y));
    harness
        .report_lines
        .push(format!("Final Y: {:.4}m", final_y));
    harness
        .report_lines
        .push(format!("Reached floor: {}", reached_floor));
...
fn test_sediment_advection() {
    ...
    harness.run_for_seconds(10.0, |h, _| {
        final_x = h.dem_sim.clumps.iter().map(|c| c.position.x).sum::<f32>()
            / h.dem_sim.clumps.len().max(1) as f32;
    });

    harness
        .report_lines
        .push(format!("Start X: {:.4}m", start_x));
    harness
        .report_lines
        .push(format!("Final X: {:.4}m", final_x));
...
fn test_sluice_riffle_capture() {
    ...
    for i in 0..5 {
        ...
        harness
            .dem_sim
            .spawn(harness.gold_template_idx, pos, flow_vel);
    }
    ...
    for i in 0..5 {
        ...
        harness
            .dem_sim
            .spawn(harness.sand_template_idx, pos, flow_vel);
    }
    ...
    harness
        .report_lines
        .push(format!("Gold avg X: {:.4}m", avg_gold_x));
    harness
        .report_lines
        .push(format!("Sand avg X: {:.4}m", avg_sand_x));
    harness
        .report_lines
        .push(format!("Gold capture rate: {:.2}", gold_capture_rate));
    harness
        .report_lines
        .push(format!("Sand capture rate: {:.2}", sand_capture_rate));
