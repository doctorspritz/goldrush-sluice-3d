...
impl ScreenSpaceFluidRenderer {
    pub fn new(device: &wgpu::Device, format: wgpu::TextureFormat) -> Self {
        ...
        let blur_direction_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
    ...
    pub fn resize(&mut self, device: &wgpu::Device, width: u32, height: u32) {
        ...
        let blur_tex_a = device.create_texture(&wgpu::TextureDescriptor { label: Some("Blur A"), ..desc });
        self.blur_view_a = Some(blur_tex_a.create_view(&Default::default()));
        self.blur_texture_a = Some(blur_tex_a);
        
        let blur_tex_b = device.create_texture(&wgpu::TextureDescriptor { label: Some("Blur B"), ..desc });
    ...
    pub fn render(
        ...
        
        {
            ...
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                ...
                color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                    ...
                    resolve_target: None,
                    ops: wgpu::Operations {
                        load: wgpu::LoadOp::Clear(wgpu::Color { r: -10000.0, g: 0.0, b: 0.0, a: 0.0 }), // Clear to far negative
        ...
        let dir_h_bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("Dir H BG"),
            layout: &dir_layout,
            entries: &[wgpu::BindGroupEntry { binding: 0, resource: dir_h.as_entire_binding() }]
        });
        ...
        let blur_bg_h = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(self.depth_view.as_ref().unwrap()) },
        ...
        
        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                ...
                color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                    ...
                    resolve_target: None,
                    ops: wgpu::Operations { load: wgpu::LoadOp::Clear(wgpu::Color { r: -10000.0, g: 0.0, b: 0.0, a: 0.0 }), store: wgpu::StoreOp::Store },
                })],
        ...
        let dir_v_bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("Dir V BG"),
            layout: &dir_layout,
            entries: &[wgpu::BindGroupEntry { binding: 0, resource: dir_v.as_entire_binding() }]
        });
        ...
        let blur_bg_v = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            layout: &self.blur_bind_group_layout,
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(self.blur_view_a.as_ref().unwrap()) },
                wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::Sampler(&self.linear_sampler) },
            ]
        });
        ...
        let compose_bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(self.blur_view_b.as_ref().unwrap()) },
                wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::Sampler(&self.linear_sampler) },
            ]
        });
