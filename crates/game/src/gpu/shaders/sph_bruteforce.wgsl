        ...
        let r2 = dot(r, r);
            ...
            if (r2 < params.h2) {
                ...
                if (r2 > 0.0001) {
                    ...
                    dii_sum += params.particle_mass * grad;
                }
            }
    }
...
fn bf_sum_dij(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var j = 0u; j < params.num_particles; j++) {
        ...
        if (r2 < params.h2 && r2 > 0.0001) {
            ...
            // d_ij = -dt² * m_j * (1/ρ_j²) * |∇W|²
            let d_ij = -params.dt2 * params.particle_mass / max(rhoj * rhoj, 0.0001) * dot(grad, grad);
            sum += d_ij * pj_pressure;
        }
    }

    pressure_accel[i] = vec4(0.0);
}
...
fn bf_apply_pressure(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var j = 0u; j < params.num_particles; j++) {
        ...
        if (r2 < params.h2 && r2 > 0.0001) {
            ...
            f_pressure -= params.particle_mass * pressure_term * grad;
