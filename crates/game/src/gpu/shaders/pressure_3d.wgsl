...
fn pressure_red(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    // -X neighbor (inlet side: Neumann BC to allow pressure buildup)
    // Using Neumann allows hydrostatic pressure to develop at inlet,
    // preventing artificial back-pressure that would kill inflow.
    if (i > 0u) {
        sum_neighbors += get_neighbor_pressure(i - 1u, j, k, p);
    } else {
        sum_neighbors += p;  // Neumann BC: use current pressure at inlet
    }

    // +X neighbor (outlet side: Dirichlet p=0 for open outlet - water can freely exit)
    if (i < params.width - 1u) {
        sum_neighbors += get_neighbor_pressure(i + 1u, j, k, p);
    } else {
        sum_neighbors += 0.0;  // Dirichlet BC: OPEN outlet (atmospheric)
    }
...
fn pressure_black(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    // -X neighbor (inlet: Neumann BC to allow pressure buildup)
    if (i > 0u) {
        sum_neighbors += get_neighbor_pressure(i - 1u, j, k, p);
    } else {
        sum_neighbors += p;  // Neumann BC: use current pressure at inlet
    ...
    // +X neighbor (outlet: Dirichlet for open outlet)
    if (i < params.width - 1u) {
        sum_neighbors += get_neighbor_pressure(i + 1u, j, k, p);
    } else {
        sum_neighbors += 0.0;  // Dirichlet BC: OPEN outlet (atmospheric)
