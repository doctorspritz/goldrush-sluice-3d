...
impl GpuBridge3D {
    pub fn new(
        ...
        let emitter_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(include_str!("shaders/particle_emitter_3d.wgsl").into()),
        });
        
        let absorption_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("3D Absorption Shader"),
            source: wgpu::ShaderSource::Wgsl(include_str!("shaders/particle_absorption_3d.wgsl").into()),
        });
        ...
        let emitter_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: emitter_params_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 1, resource: count_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 2, resource: flip.positions_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 3, resource: flip.velocities_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 4, resource: flip.densities_buffer.as_entire_binding() },
            ],
        });

        let absorption_bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        ...
        let absorption_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: absorption_params_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 1, resource: flip.positions_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 2, resource: flip.velocities_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 3, resource: flip.densities_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 4, resource: flip.bed_height_buffer().as_entire_binding() },
                wgpu::BindGroupEntry { binding: 5, resource: transfer_sediment_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 6, resource: transfer_water_buffer.as_entire_binding() },
            ],
        });
        ...
        let absorption_pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("Absorption 3D Pipe Layout"),
            bind_group_layouts: &[&absorption_bind_group_layout],
            push_constant_ranges: &[],
        });
        
        let absorption_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
    ...
    pub fn dispatch_absorption(
        ...
        queue.write_buffer(&self.absorption_params_buffer, 0, bytemuck::bytes_of(&params));
