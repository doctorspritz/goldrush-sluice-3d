...
impl GpuParticleSort {
    ...
    pub fn new(
        ...
        let keys_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(include_str!("shaders/particle_sort_keys.wgsl").into()),
        });

        let count_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Particle Sort Count Shader"),
            source: wgpu::ShaderSource::Wgsl(include_str!("shaders/particle_sort_count.wgsl").into()),
        });

        let prefix_sum_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Particle Sort Prefix Sum Shader"),
            source: wgpu::ShaderSource::Wgsl(include_str!("shaders/particle_sort_prefix_sum.wgsl").into()),
        });

        let scatter_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Particle Sort Scatter Shader"),
            source: wgpu::ShaderSource::Wgsl(include_str!("shaders/particle_sort_scatter.wgsl").into()),
        });

        // Create bind group layouts and bind groups
        // Keys bind group
        let keys_bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        ...
        let compute_keys_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: sort_params_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 1, resource: in_positions_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 2, resource: cell_keys_buffer.as_entire_binding() },
            ],
        });
        ...
        let count_cells_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: sort_params_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 1, resource: cell_keys_buffer.as_entire_binding() },
        ...
        let prefix_sum_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: prefix_sum_params_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 1, resource: cell_offsets_buffer.as_entire_binding() },
                ...
                wgpu::BindGroupLayoutEntry {
                    ...
                    ty: wgpu::BindingType::Buffer {
                        ty: if i == 0 {
                            ...
                            wgpu::BufferBindingType::Storage { read_only: i < 3 || (i >= 4 && i < 10) }
                        },
                        has_dynamic_offset: false,
                        min_binding_size: None,
                    },
        ...
        let scatter_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: sort_params_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 1, resource: cell_keys_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 2, resource: cell_offsets_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 3, resource: cell_counters_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 4, resource: in_positions_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 5, resource: in_velocities_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 6, resource: in_densities_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 7, resource: in_c_col0_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 8, resource: in_c_col1_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 9, resource: in_c_col2_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 10, resource: out_positions_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 11, resource: out_velocities_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 12, resource: out_densities_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 13, resource: out_c_col0_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 14, resource: out_c_col1_buffer.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 15, resource: out_c_col2_buffer.as_entire_binding() },
            ],
        });
        ...
        let count_cells_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        ...
        let scan_block_sums_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        ...
        let add_block_offsets_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
    ...
    pub fn prepare(&self, queue: &wgpu::Queue, particle_count: u32, cell_size: f32) {
        ...
        queue.write_buffer(&self.prefix_sum_params_buffer, 0, bytemuck::bytes_of(&prefix_sum_params));
    ...
    pub fn encode(&self, encoder: &mut wgpu::CommandEncoder, queue: &wgpu::Queue, particle_count: u32) {
