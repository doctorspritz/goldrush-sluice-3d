...
const TESTS: &[TestDef] = &[
    TestDef { key: '1', name: "Floor Collision", unit_test: "test_dem_floor_collision", expect: "Bounce ~4cm then settle", watch: "min_y >= 0.02" },
    TestDef { key: '2', name: "Wall Collision", unit_test: "test_dem_wall_collision", expect: "Hit wall, bounce back", watch: "v_x flips sign" },
    TestDef { key: '3', name: "Clump Collision", unit_test: "test_dem_clump_collision", expect: "Head-on, both reverse", watch: "momentum ~0" },
    TestDef { key: '4', name: "No Penetration", unit_test: "test_dem_no_penetration", expect: "100 clumps, none through floor", watch: "penetrations = 0" },
    TestDef { key: '5', name: "Static Friction", unit_test: "test_dem_static_friction", expect: "At rest stays still", watch: "drift < 2cm" },
    TestDef { key: '6', name: "Kinetic Friction", unit_test: "test_dem_kinetic_friction", expect: "Sliding slows down", watch: "v < 50% initial" },
    TestDef { key: '7', name: "Wet vs Dry", unit_test: "test_dem_wet_vs_dry", expect: "Blue(wet) slides 2x+ farther", watch: "wet/dry > 2" },
    TestDef { key: '8', name: "Settling", unit_test: "test_dem_settling_time", expect: "All settle in 5s", watch: "avg_vel < 0.05" },
    TestDef { key: '9', name: "Density Separation", unit_test: "test_dem_density_separation", expect: "Gold(yellow) sinks below gray", watch: "gold_y < gray_y" },
];
...
impl App {
    ...
    fn update_title(&self) {
        if let Some(w) = &self.window {
            ...
            let title = format!("TEST {}: {} | EXPECT: {} | WATCH: {}{}",
    ...
    fn setup_test(&mut self, idx: usize) {
        ...
        match idx {
            ...
            5 => { // Kinetic friction
                sim.spawn(tpl_idx, Vec3::new(0.3, RADIUS * 2.0, 1.0), Vec3::new(2.0, 0.0, 0.0));
                self.initial_vel = 2.0;
            }
            6 => { // Wet vs dry
                sim.wet_friction = 0.08;
                sim.floor_friction = 0.5;
                sim.spawn(tpl_idx, Vec3::new(0.3, RADIUS * 2.0, 0.8), Vec3::new(1.5, 0.0, 0.0));
                ...
                dry.spawn(dry_tpl_idx, Vec3::new(0.3, RADIUS * 2.0, 1.2), Vec3::new(1.5, 0.0, 0.0));
            ...
            8 => { // Density separation
                ...
                for i in 0..10 {
                    ...
                    let gi = sim.spawn(gold_idx, Vec3::new(x, 1.5 + i as f32 * 0.06, z), Vec3::ZERO);
                    self.gold_indices.push(gi);
                    let si = sim.spawn(tpl_idx, Vec3::new(x + 0.06, 1.55 + i as f32 * 0.06, z + 0.06), Vec3::ZERO);
        ...
        match self.test_idx {
            0 => {
                if let Some(c) = self.sim.clumps.first() {
                    ...
                    println!("y={:.4} vy={:.3} max_bounce={:.4} {}",
                        c.position.y, c.velocity.y, self.max_bounce_y,
            ...
            1 => {
                if let Some(c) = self.sim.clumps.first() {
                    ...
                    println!("x={:.3} vx={:.3} {}", c.position.x, c.velocity.x,
            ...
            2 => {
                if self.sim.clumps.len() >= 2 {
                    ...
                    println!("v_a={:.3} v_b={:.3} momentum={:.4} {}", va, vb, p,
                        if ok { "✓ BOTH REVERSED" } else { "" });
                }
            }
            3 => {
                let min_y = self.sim.clumps.iter().map(|c| c.position.y).fold(f32::MAX, f32::min);
                let bad = self.sim.clumps.iter().filter(|c| c.position.y < RADIUS * 0.9).count();
                println!("min_y={:.4} penetrations={} {}", min_y, bad,
                    if bad == 0 { "✓ OK" } else { "✗ FAIL" });
            }
            4 => {
                if let Some(c) = self.sim.clumps.first() {
                    let drift = (c.position.x - 1.0).hypot(c.position.z - 1.0);
                    println!("drift={:.4}m {}", drift, if drift < 0.02 { "✓ STABLE" } else { "" });
                }
            }
            5 => {
                if let Some(c) = self.sim.clumps.first() {
                    let ratio = c.velocity.x / self.initial_vel;
                    println!("vx={:.3} ratio={:.2} {}", c.velocity.x, ratio,
                        if ratio < 0.5 { "✓ DECELERATED" } else { "" });
                }
            }
            6 => {
                let wet_d = self.sim.clumps.first().map(|c| c.position.x - self.wet_start_x).unwrap_or(0.0);
                let dry_d = self.sim_dry.as_ref().and_then(|s| s.clumps.first()).map(|c| c.position.x - self.dry_start_x).unwrap_or(0.001);
                let ratio = wet_d / dry_d.max(0.001);
                println!("wet={:.3}m dry={:.3}m ratio={:.2}x {}", wet_d, dry_d, ratio,
                    if ratio > 2.0 { "✓ WET SLIDES MORE" } else { "" });
            }
            7 => {
                let avg: f32 = self.sim.clumps.iter().map(|c| c.velocity.length()).sum::<f32>() / self.sim.clumps.len().max(1) as f32;
                let max = self.sim.clumps.iter().map(|c| c.velocity.length()).fold(0.0f32, f32::max);
                println!("avg_vel={:.4} max_vel={:.4} {}", avg, max,
                    if avg < 0.05 && max < 0.1 { "✓ SETTLED" } else { "" });
            }
            8 => {
                let gold_y: f32 = self.gold_indices.iter().filter_map(|&i| self.sim.clumps.get(i)).map(|c| c.position.y).sum::<f32>() / self.gold_indices.len().max(1) as f32;
                let gang_y: f32 = self.gangue_indices.iter().filter_map(|&i| self.sim.clumps.get(i)).map(|c| c.position.y).sum::<f32>() / self.gangue_indices.len().max(1) as f32;
                let sep = gang_y - gold_y;
                println!("gold_y={:.3} gangue_y={:.3} sep={:.3}m {}", gold_y, gang_y, sep,
    ...
    fn step(&mut self) {
        ...
        for _ in 0..SUBSTEPS {
            self.sim.step(DT);
            if let Some(dry) = &mut self.sim_dry { dry.step(DT); }
        }
        self.frame += 1;
...
struct Vertex { pos: [f32; 3], norm: [f32; 3] }

#[repr(C)]
#[derive(Clone, Copy, Pod, Zeroable)]
struct Instance { pos: [f32; 3], scale: f32, color: [f32; 4] }
            ...
            WindowEvent::KeyboardInput { event, .. } if event.state.is_pressed() => {
                if let PhysicalKey::Code(k) = event.physical_key {
                    ...
                    match k {
                        ...
                        KeyCode::Space => { self.paused = !self.paused; self.update_title(); }
...
impl App {
    async fn init_gpu(&mut self, window: Arc<Window>) {
        ...
        let floor_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: None, layout: Some(&layout),
            vertex: wgpu::VertexState {
                module: &shader, entry_point: Some("vs_floor"),
                buffers: &[wgpu::VertexBufferLayout { array_stride: 24, step_mode: wgpu::VertexStepMode::Vertex, attributes: &wgpu::vertex_attr_array![0 => Float32x3, 1 => Float32x3] }],
                compilation_options: Default::default(),
            },
            fragment: Some(wgpu::FragmentState {
                module: &shader, entry_point: Some("fs_floor"),
                targets: &[Some(caps.formats[0].into())],
                compilation_options: Default::default(),
            }),
            primitive: Default::default(),
        ...
        let mesh_verts = device.create_buffer_init(&wgpu::util::BufferInitDescriptor { label: None, contents: bytemuck::cast_slice(&sphere), usage: wgpu::BufferUsages::VERTEX });
        let floor_verts = device.create_buffer_init(&wgpu::util::BufferInitDescriptor { label: None, contents: bytemuck::cast_slice(&floor), usage: wgpu::BufferUsages::VERTEX });
        let instances = device.create_buffer(&wgpu::BufferDescriptor { label: None, size: 32 * 500, usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false });
        let uniforms = device.create_buffer(&wgpu::BufferDescriptor { label: None, size: 64, usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false });
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor { label: None, layout: &bgl, entries: &[wgpu::BindGroupEntry { binding: 0, resource: uniforms.as_entire_binding() }] });
        ...
        self.gpu = Some(GpuState {
            surface, device, queue, config, pipeline, floor_pipeline,
            mesh_verts, mesh_count: sphere.len() as u32,
            floor_verts, floor_count: floor.len() as u32,
            instances, uniforms, bind_group,
    ...
    fn render(&mut self) {
        ...
        let gpu = match self.gpu.take() { Some(g) => g, None => return };

        let eye = self.camera_pos();
        let view = Mat4::look_at_rh(eye, self.camera_target, Vec3::Y);
        let size = window.inner_size();
        let proj = Mat4::perspective_rh(45f32.to_radians(), size.width as f32 / size.height.max(1) as f32, 0.1, 100.0);
        gpu.queue.write_buffer(&gpu.uniforms, 0, bytemuck::bytes_of(&Uniforms { view_proj: (proj * view).to_cols_array_2d() }));
        ...
        for (i, c) in self.sim.clumps.iter().enumerate() {
            ...
            insts.push(Instance { pos: c.position.to_array(), scale: RADIUS, color });
        }
        if let Some(dry) = &self.sim_dry {
            for c in &dry.clumps {
                insts.push(Instance { pos: c.position.to_array(), scale: RADIUS, color: [0.9, 0.5, 0.2, 1.0] }); // Dry - orange
            }
        }

        if !insts.is_empty() {
            gpu.queue.write_buffer(&gpu.instances, 0, bytemuck::cast_slice(&insts));
        }

        let out = match gpu.surface.get_current_texture() {
            Ok(t) => t,
            Err(_) => { self.gpu = Some(gpu); return; }
        };
...
fn make_sphere(sub: u32) -> Vec<Vertex> {
    ...
    let v: Vec<Vec3> = [(-1.0,phi,0.0),(1.0,phi,0.0),(-1.0,-phi,0.0),(1.0,-phi,0.0),(0.0,-1.0,phi),(0.0,1.0,phi),(0.0,-1.0,-phi),(0.0,1.0,-phi),(phi,0.0,-1.0),(phi,0.0,1.0),(-phi,0.0,-1.0),(-phi,0.0,1.0)]
        .iter().map(|&(x,y,z)| Vec3::new(x,y,z).normalize()).collect();
    let f = [(0,11,5),(0,5,1),(0,1,7),(0,7,10),(0,10,11),(1,5,9),(5,11,4),(11,10,2),(10,7,6),(7,1,8),(3,9,4),(3,4,2),(3,2,6),(3,6,8),(3,8,9),(4,9,5),(2,4,11),(6,2,10),(8,6,7),(9,8,1)];
    let mut out = Vec::new();
    fn subdiv(a: Vec3, b: Vec3, c: Vec3, d: u32, o: &mut Vec<Vertex>) {
        if d == 0 { for p in [a,b,c] { o.push(Vertex { pos: p.to_array(), norm: p.to_array() }); } }
        else {
            let ab = ((a+b)/2.0).normalize(); let bc = ((b+c)/2.0).normalize(); let ca = ((c+a)/2.0).normalize();
            subdiv(a,ab,ca,d-1,o); subdiv(b,bc,ab,d-1,o); subdiv(c,ca,bc,d-1,o); subdiv(ab,bc,ca,d-1,o);
        }
    }
    for (i,j,k) in f { subdiv(v[i],v[j],v[k],sub,&mut out); }
    out
}
...
fn make_floor(s: f32) -> Vec<Vertex> {
    let n = [0.0,1.0,0.0];
    vec![
        Vertex { pos: [-s,0.0,-s], norm: n }, Vertex { pos: [s,0.0,-s], norm: n }, Vertex { pos: [s,0.0,s], norm: n },
        Vertex { pos: [-s,0.0,-s], norm: n }, Vertex { pos: [s,0.0,s], norm: n }, Vertex { pos: [-s,0.0,s], norm: n },
    ]
}
