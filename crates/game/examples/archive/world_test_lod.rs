...
impl App {
            ...
            if let Some(hit) = self.raycast_terrain() {
                ...
                if let Some(gpu) = self.gpu.as_ref() {
                    for tile in tool_tiles {
                        ...
                        if is_adding {
                            gpu.heightfield.dispatch_material_tool_tile(
                                &mut encoder,
                                tile.width,
                                tile.depth,
                            );
                        } else {
                            gpu.heightfield.dispatch_excavate_tile(
                                &mut encoder,
                                tile.width,
                                tile.depth,
                            );
    ...
    fn record_activity(&mut self, position: Vec3, intensity: f32) {
        self.activity_points.push(ActivityPoint {
            position,
            intensity,
        });
    }
    ...
    fn select_tiles(&self) -> Vec<TileRegion> {
        ...
        let mut scores: std::collections::HashMap<TileCoord, f32> =
        ...
        for source in sources {
            ...
            for dz in -radius_tiles..=radius_tiles {
                for dx in -radius_tiles..=radius_tiles {
                    ...
                    let dist =
                        (center - Vec3::new(source.position.x, 0.0, source.position.z)).length();
    ...
    fn tile_region(&self, coord: TileCoord) -> Option<TileRegion> {
        ...
        let width = (self.world.width as u32)
            ...
            .min(TILE_SIZE);
        let depth = (self.world.depth as u32)
    ...
    fn render(&mut self) {
        ...
        let mut encoder = gpu.device.create_command_encoder(&Default::default());
        {
            ...
            pass.set_index_buffer(
                gpu.overlay.index_buffer.slice(..),
                wgpu::IndexFormat::Uint32,
            );
