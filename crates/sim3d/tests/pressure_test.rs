...
fn test_divergence_includes_w_component() {
    ...
    sim.grid.cell_type_mut()[center_idx] = CellType::Fluid;
    ...
    sim.grid.w_mut()[w_back] = 0.0;
    sim.grid.w_mut()[w_front] = 1.0; // Flow exiting in +Z direction
...
fn test_z_boundary_conditions() {
    ...
    for j in 0..8 {
        for i in 0..8 {
            ...
            assert_eq!(sim.grid.w_mut()[idx0], 0.0, "W at k=0 should be zero");
            assert_eq!(sim.grid.w_mut()[idx_depth], 0.0, "W at k=depth should be zero");
        }
    }
    ...
    assert_eq!(
        sim.grid.w_mut()[interior_idx], 5.0,
...
fn test_pressure_affects_all_components() {
    ...
    for k in 2..6 {
        for j in 2..6 {
            let idx = sim.grid.u_index(2, j, k);
            if sim.grid.u()[idx].abs() > 0.01 {
    ...
    for k in 2..6 {
        for i in 2..6 {
            let idx = sim.grid.v_index(i, 2, k);
            if sim.grid.v()[idx].abs() > 0.01 {
    ...
    for j in 2..6 {
        for i in 2..6 {
            let idx = sim.grid.w_index(i, j, 2);
            if sim.grid.w()[idx].abs() > 0.01 {
