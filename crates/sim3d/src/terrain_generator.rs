...
pub fn generate_klondike_terrain(
    ...
    for z in 0..depth {
        ...
        for x in 0..width {
            ...
            let paydirt_variation = paydirt_noise.get([world_x as f64 * 0.02, world_z as f64 * 0.02]) as f32;
            let paydirt = (paydirt_base + paydirt_variation * 0.5).max(0.1).min(config.max_paydirt);
            ...
            let overburden_variation = overburden_noise.get([world_x as f64 * 0.01, world_z as f64 * 0.01]) as f32;
...
pub fn get_creek_source(width: usize, depth: usize, cell_size: f32, config: &TerrainConfig) -> (f32, f32, f32) {
    let valley_center_x = (width as f32 * cell_size) / 2.0;
